...
export const ChapterSlideForm = ({
    ...
    try {
        ...
        if (!pdfUrl) {
            toast.error('PDF URL is req4444444444uired');
            return;
        }
        ...
        if (response.status === 200) {
            ...
            const updateVideoUrlResponse = await axios.post(
                '/api/updateVideoUrl',
                { chapterId, videoUrl: videoPath },
                {
                    headers: {
            ...
            if (updateVideoUrlResponse.status === 200) {
